...
export default function LeadsPage() {
  ...
  // Simple OpenStreetMap iframe for map view
  const mapVendors = (filteredVendors || []).filter(v => v.lat && v.lng).slice(0, 50);
        ...
        {view === 'map' && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="absolute top-4 right-4 max-h-80 overflow-y-auto space-y-2 w-64" style={{ scrollbarWidth: 'thin' }}>
                ...
                {(filteredVendors || []).length > 10 && (
                  <div className="bg-violet-50 rounded-xl p-3 text-center text-xs font-bold text-violet-600 border border-violet-100">
                    +{(filteredVendors.length - 10).toLocaleString()} more leads
                  </div>
                )}
                {(filteredVendors || []).length === 0 && (
                  <div className="bg-white rounded-xl p-4 text-center text-xs text-gray-400 border border-gray-100">
