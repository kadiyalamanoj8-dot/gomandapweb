...
function App() {
  ...
  const [activeTab, setActiveTab] = useState('staging'); // staging, pushed
  ...
  const startScrape = async () => {
    ...
    setLoading(true);
    try {
      // If the user selects "All Categories" or "All Mandals", automatically use the batch engine
      const useBatchEngine = isAutoPilot || category === 'All Categories' || mandal === 'All Mandals';
      
      if (useBatchEngine) {
        await axios.post(`${API_URL}/scrape/batch`, { district, category, mandal, engine });
      } else {
        await axios.post(`${API_URL}/scrape`, { category, district, mandal, engine });
      }
  ...
  const exportToCSV = () => {
    const dataToExport = activeTab === 'staging' ? stagingVendors : liveVendors;
  ...
  const displayedVendors = (activeTab === 'staging' ? stagingVendors : liveVendors).filter(v => 
              ...
              <div className="flex bg-black/50 border border-white/10 rounded-xl p-1 mb-2">
                <button 
                  onClick={() => {
                    ...
                    if (category === 'All Categories') setCategory('Banquet Halls');
                    if (mandal === 'All Mandals') setMandal(regionsData[district]?.[0] || '');
                  }}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <ApplePicker
                    ...
                    onChange={val => {
                      setDistrict(val);
                      setMandal(isAutoPilot ? 'All Mandals' : (regionsData[val]?.[0] || ''));
                    }}
                ...
                <div>
                  ...
                  <ApplePicker
                    ...
                    className="w-full"
                  />
                  {isAutoPilot && mandal === 'All Mandals' && (
                    <div className="text-[10px] text-[#D4AF37] mt-1.5 font-bold">Auto-loops all mandals in {district}</div>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={`flex items-center gap-2 font-bold px-4 py-2 rounded-lg transition-colors ${activeTab === 'staging' ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white'}`}
                >
                  <LayoutDashboard size={18}/> Staging Queue
                  <span className="bg-white/10 text-white text-[10px] px-2 py-0.5 rounded-full ml-1">{stagingVendors.length}</span>
                </button>
                <button 
                  onClick={() => setActiveTab('pushed')}
                  className={`flex items-center gap-2 font-bold px-4 py-2 rounded-lg transition-colors ${activeTab === 'pushed' ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white'}`}
                >
                  <Database size={18}/> Live Database
                  <span className="bg-white/10 text-white text-[10px] px-2 py-0.5 rounded-full ml-1">{liveVendors.length}</span>
                </button>
              </div>
