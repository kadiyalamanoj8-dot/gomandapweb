...
async function scrapeWeddingWire(category, location) {
  ...
  deps.logger(`[WeddingWire Engine] Starting scrape for ${category} in ${location}`);
  ...
  try {
    ...
    deps.logger(`[WeddingWire Engine] Found ${results.length} raw results.`);
    let newCount = 0;
    ...
    deps.logger(`[WeddingWire Engine] Complete. Inserted ${newCount} new vendors.`);
  } catch (error) {
    deps.logger(`[WeddingWire Error] ${error.message}`);
  } finally {
