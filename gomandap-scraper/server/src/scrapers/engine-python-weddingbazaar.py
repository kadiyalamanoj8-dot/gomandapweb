...
def main():
    parser = argparse.ArgumentParser(description="Gomandap WeddingBazaar Engine")
    ...
    emit_log("[SYSTEM] Python WeddingBazaar Automation Booted.")
    ...
    search_url = f"https://www.weddingbazaar.com/vendors/{safe_location}/{safe_query}"

    emit_log(f"[Agent] Navigating to WeddingBazaar: {search_url}")
    ...
    with sync_playwright() as p:
        ...
        cards = page.locator('.vendor-card, [data-testid="vendor-card"]').all()
        
        if not cards:
            emit_log("[WARN] Standard vendor cards not found. Checking links...")
            cards = page.locator('a[href*="/wedding-vendors/"]').all()
            
        emit_log(f"[Agent] Found {len(cards)} vendor cards on WeddingBazaar.")

        for i, card in enumerate(cards):
            try:
                name = ""
                name_el = card.locator('h3, .vendor-name').first
                if name_el.count() > 0:
                ...
                address = ""
                addr_el = card.locator('.vendor-location, .location').first
                if addr_el.count() > 0:
                ...
                lead = {
                    ...
                    "mapsLink": page.url,
                    "qualityScore": score,
                    "source": "Python WeddingBazaar Automation"
                }
        ...
        emit_log("[SYSTEM] Python WeddingBazaar Automation completed successfully.")
        browser.close()
